...
lemma image_nonempty {f : EuclideanSpace ℝ (Fin d) → ℝ} {A : Finset (EuclideanSpace ℝ (Fin d))} (ha : A.Nonempty) : (A.image f).Nonempty :=
  (Finset.image_nonempty).mpr ha

theorem reject_iff_ball (f : EuclideanSpace ℝ (Fin d) → ℝ) {A : Finset (EuclideanSpace ℝ (Fin d))} (hA : A.Nonempty) {κ : ℝ} (hκ : 0 < κ) (x : EuclideanSpace ℝ (Fin d)) :
...
example (x₁ x₂ : EuclideanSpace ℝ (Fin d)) (r₁ r₂ : ℝ) (h : r₁ ≤ r₂) : volume (ball x₁ r₁) ≤ volume (ball x₂ r₂) := by
